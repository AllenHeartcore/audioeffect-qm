from typing import Union
import warnings

import numpy as np
import zlib


def read_encfile(filename: str) -> tuple:

    toint = lambda x: int.from_bytes(x, byteorder="big")

    with open(filename, "rb") as fin:

        magic = fin.read(4)
        assert magic == b"NCAE"

        ldata = toint(fin.read(4))
        fin.read(8)  # unused

        lkey = toint(fin.read(1)) - 1
        assert lkey % 4 == 0
        key0 = bytearray(fin.read(lkey + 1))
        key = np.array(key0[:4] + key0[5:]) ^ key0[4]

        data = bytearray(fin.read(ldata))

    return key, data


class NCAEDecryptor:

    def __init__(self, key: Union[bytes, bytearray, np.ndarray]):

        table = np.arange(0x100, dtype=np.uint8)
        byte = 0

        for i in range(0x100):
            byte = table[i] + key[i % len(key)] + byte & 0xFF
            table[i], table[byte] = table[byte], table[i]

        self._table = table

    def decrypt(self, data: Union[bytes, bytearray, np.ndarray]) -> bytes:

        for j in range(len(data)):
            byte = self._table[(j + 1) & 0xFF]
            data[j] ^= self._table[self._table[(byte + j + 1) & 0xFF] + byte & 0xFF]

        return zlib.decompress(data, -zlib.MAX_WBITS)

def func(offset, size, data):
    key = [,] 
    str_len = len(key)

    # 如果 offset 为负数，返回 -1
    if offset < 0:
        return -1

    # 如果 size <= 0，返回 size
    if size <= 0:
        return size

    data = bytearray(data)  # 将输入数据转换为可变字节数组

    for i in range(size):
        index = offset + i

        # 确保索引在长度范围内
        index = ((index * index + 27) % str_len)

        xor_value = key[index]

        data[i] ^= xor_value  # 对应的字符值进行 XOR 操作

    return data

with open('ugc/4408-极品均衡.qmaep','rb') as f:
    data = f.read()
result = func(0, len(data), data)
with open('result.bin', 'wb') as f:
    f.write(result)
print(result)
